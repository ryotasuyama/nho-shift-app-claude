import { NextRequest } from "next/server";
import { prisma } from "@/lib/prisma";
import { requireAuth } from "@/lib/api/auth-guard";
import { successResponse, errorResponse } from "@/lib/api/response";
import { ERROR_CODES } from "@/lib/api/errors";
import { createRequestSchema } from "@/lib/validators/request";
import { parseDate, formatDate, isAfterDeadline } from "@/lib/utils/date";

type RouteContext = { params: Promise<{ termId: string }> };

const MAX_REQUESTS_PER_TERM = 3;

export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    const { user, error: authError } = await requireAuth();
    if (authError) return authError;

    const { termId } = await context.params;

    const term = await prisma.term.findUnique({ where: { id: termId } });
    if (!term) {
      return errorResponse(ERROR_CODES.NOT_FOUND.code, "タームが見つかりません", ERROR_CODES.NOT_FOUND.status);
    }

    const isAdmin = user!.role === "admin";

    // Staff can only see collecting/confirmed terms
    if (!isAdmin && term.status !== "collecting" && term.status !== "confirmed") {
      return errorResponse(ERROR_CODES.FORBIDDEN.code, "このタームにアクセスする権限がありません", ERROR_CODES.FORBIDDEN.status);
    }

    // Build where clause based on role
    const where: { term_id: string; staff_id?: string } = { term_id: termId };

    if (!isAdmin) {
      // Staff: only their own requests
      const staff = await prisma.staff.findUnique({
        where: { user_id: user!.userId },
        select: { id: true },
      });
      if (!staff) {
        return errorResponse(ERROR_CODES.NOT_FOUND.code, "スタッフ情報が見つかりません", ERROR_CODES.NOT_FOUND.status);
      }
      where.staff_id = staff.id;
    }

    const requests = await prisma.shiftRequest.findMany({
      where,
      include: { staff: { select: { name: true } } },
      orderBy: { requested_date: "asc" },
    });

    const formatted = requests.map((r) => ({
      id: r.id,
      staff_id: r.staff_id,
      staff_name: r.staff.name,
      requested_date: formatDate(r.requested_date),
      created_at: r.created_at.toISOString(),
    }));

    return successResponse(formatted);
  } catch (error) {
    console.error("GET /api/terms/[termId]/requests error:", error);
    return errorResponse(ERROR_CODES.INTERNAL_ERROR.code, "サーバー内部エラー", ERROR_CODES.INTERNAL_ERROR.status);
  }
}

export async function POST(request: Request, context: RouteContext) {
  try {
    const { user, error: authError } = await requireAuth();
    if (authError) return authError;

    const { termId } = await context.params;

    const body: unknown = await request.json();
    const result = createRequestSchema.safeParse(body);
    if (!result.success) {
      return errorResponse(
        ERROR_CODES.VALIDATION_ERROR.code,
        "入力値が不正です",
        ERROR_CODES.VALIDATION_ERROR.status,
        result.error.issues.map((i) => ({ field: i.path.join("."), message: i.message }))
      );
    }

    const { staff_id, requested_date } = result.data;
    const isAdmin = user!.role === "admin";

    // Check term exists and is collecting
    const term = await prisma.term.findUnique({ where: { id: termId } });
    if (!term) {
      return errorResponse(ERROR_CODES.NOT_FOUND.code, "タームが見つかりません", ERROR_CODES.NOT_FOUND.status);
    }
    if (term.status !== "collecting") {
      return errorResponse(ERROR_CODES.CONFLICT.code, "このタームは希望休を受け付けていません", ERROR_CODES.CONFLICT.status);
    }

    // Staff: verify staff_id matches JWT user
    if (!isAdmin) {
      const staffRecord = await prisma.staff.findUnique({
        where: { user_id: user!.userId },
        select: { id: true },
      });
      if (!staffRecord || staffRecord.id !== staff_id) {
        return errorResponse(ERROR_CODES.FORBIDDEN.code, "他のスタッフの希望休は申請できません", ERROR_CODES.FORBIDDEN.status);
      }

      // Staff: check deadline
      if (term.request_deadline && isAfterDeadline(term.request_deadline)) {
        return errorResponse(ERROR_CODES.FORBIDDEN.code, "申請受付は終了しています", ERROR_CODES.FORBIDDEN.status);
      }
    }

    // Check staff is active
    const staff = await prisma.staff.findUnique({
      where: { id: staff_id },
      select: { id: true, is_active: true, name: true },
    });
    if (!staff) {
      return errorResponse(ERROR_CODES.NOT_FOUND.code, "スタッフが見つかりません", ERROR_CODES.NOT_FOUND.status);
    }
    if (!staff.is_active) {
      return errorResponse(ERROR_CODES.CONFLICT.code, "無効化されたスタッフの希望休は申請できません", ERROR_CODES.CONFLICT.status);
    }

    // Check date is within term period
    const reqDate = parseDate(requested_date);
    if (reqDate < term.start_date || reqDate > term.end_date) {
      return errorResponse(ERROR_CODES.VALIDATION_ERROR.code, "選択された日付はターム期間外です", ERROR_CODES.VALIDATION_ERROR.status);
    }

    // Check max 3 requests per staff per term
    const existingCount = await prisma.shiftRequest.count({
      where: { staff_id, term_id: termId },
    });
    if (existingCount >= MAX_REQUESTS_PER_TERM) {
      return errorResponse(ERROR_CODES.CONFLICT.code, "希望休は最大3日までです", ERROR_CODES.CONFLICT.status);
    }

    // Check duplicate date
    const duplicate = await prisma.shiftRequest.findUnique({
      where: { uk_shift_requests: { staff_id, term_id: termId, requested_date: reqDate } },
    });
    if (duplicate) {
      return errorResponse(ERROR_CODES.CONFLICT.code, "この日付は既に申請済みです", ERROR_CODES.CONFLICT.status);
    }

    const newRequest = await prisma.shiftRequest.create({
      data: {
        staff_id,
        term_id: termId,
        requested_date: reqDate,
      },
      include: { staff: { select: { name: true } } },
    });

    return successResponse(
      {
        id: newRequest.id,
        staff_id: newRequest.staff_id,
        staff_name: newRequest.staff.name,
        requested_date: formatDate(newRequest.requested_date),
        created_at: newRequest.created_at.toISOString(),
      },
      201
    );
  } catch (error) {
    console.error("POST /api/terms/[termId]/requests error:", error);
    return errorResponse(ERROR_CODES.INTERNAL_ERROR.code, "サーバー内部エラー", ERROR_CODES.INTERNAL_ERROR.status);
  }
}
